document.addEventListener("DOMContentLoaded", function() {
    ...
    .then(data => {
      ...
      document.getElementById('single-news-content-ar').innerHTML = data.content.ar;
      document.getElementById('single-news-author-ar').innerHTML = data.author.ar;
      document.getElementById('single-news-date-ar').innerHTML = new Date(data.published_at).toLocaleDateString('ar-EG');
      if (data.featured_image) {
        document.getElementById('single-news-image-ar').src = data.featured_image;
      }
    })
    .catch(error => {
      console.error('Error fetching single news (Arabic):', error);
      document.getElementById('single-news-loading-ar').innerHTML = 'فشل تحميل الخبر';
    });
    ...
    .then(data => {
      ...
      latestNews.forEach(news => {
        const newsItem = `
          <div class="post-mini">
            <div class="unit">
             <a href="single-news-ar.html?id=${news.id}">
              <div class="unit-left">
    ...
    .catch(error => {
      ...
      document.getElementById('latest-news-loading-ar').innerHTML = 'فشل تحميل الأخبار';
    });
  });